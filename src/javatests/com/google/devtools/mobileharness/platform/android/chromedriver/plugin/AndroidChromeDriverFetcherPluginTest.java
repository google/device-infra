...
public final class AndroidChromeDriverFetcherPluginTest {
  ...
  @Test
  public void isEnabled_withDecorator_returnsTrue() {
    when(jobInfo.type())
        .thenReturn(
            JobType.newBuilder().addDecorator("AndroidChromeDriverProviderDecorator").build());
    assertThat(AndroidChromeDriverFetcherPlugin.isEnabled(jobInfo)).isTrue();
  }

  @Test
  public void isEnabled_withoutDecorator_returnsFalse() {
    when(jobInfo.type()).thenReturn(JobType.newBuilder().build());
    assertThat(AndroidChromeDriverFetcherPlugin.isEnabled(jobInfo)).isFalse();
  }

  @Test
  public void onTestStarting_decoratorNotPresent_doesNothing() throws Exception {
    when(jobInfo.type()).thenReturn(JobType.newBuilder().build());

    plugin.onTestStarting(event);

    verify(testFiles, never()).add(any(String.class), any(String.class));
  }

  @Test
  public void onTestStarting_noVersionFound_defersToLabDecorator() throws Exception {
    when(jobInfo.type())
        .thenReturn(
            JobType.newBuilder().addDecorator("AndroidChromeDriverProviderDecorator").build());
    when(allocation.getAllDevices()).thenReturn(ImmutableMap.of());
