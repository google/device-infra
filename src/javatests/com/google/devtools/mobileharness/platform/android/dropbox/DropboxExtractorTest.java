...
public class DropboxExtractorTest {
  ...
  public void setUp() throws Exception {
    ...
    when(adb.runShell(
            ...
            any(Duration.class),
            any(DropboxParser.class)))
        .thenAnswer(
            invocation -> {
              DropboxParser parser = invocation.getArgument(3);
              DROPBOX_APP_CRASH_OUTPUT.forEach(parser::onLine);
              return "";
            });
  }
  ...
  public void extract_filtersOnTimestamp() throws Exception {
    dropboxExtractor.extract(
        "device_id",
        ImmutableList.of("com.test.app.one", "com.test.app.two"),
        ImmutableList.of(DropboxTag.DATA_APP_CRASH),
  ...
  public void extract_filtersOnProcessName() throws Exception {
    dropboxExtractor.extract(
        "device_id",
        ImmutableList.of("com.test.app.two"),
        ImmutableList.of(DropboxTag.DATA_APP_CRASH),
  ...
  public void extract_multipleDifferentCrashes() throws Exception {
    when(adb.runShell(
        ...
        .thenAnswer(
            invocation -> {
              DropboxParser parser = invocation.getArgument(3);
              DROPBOX_TOMBSTONE_OUTPUT.forEach(parser::onLine);
              return "";
            });

    dropboxExtractor.extract(
        "device_id",
        ImmutableList.of("com.test.app.one", "com.test.app.nativecrasher"),
        ImmutableList.of(DropboxTag.DATA_APP_CRASH, DropboxTag.SYSTEM_TOMBSTONE),
