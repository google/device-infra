...
public final class AndroidFileUtilTest {
  ...
  public void makeFileExecutableReadableNonWritable() throws Exception {
    ...
    assertThat(androidFileUtil.makeFileExecutableReadableNonWritable(SERIAL, filePathOnDevice))
        .isEqualTo("OK");
  }

  @Test
  public void makeFileExecutableReadableNonWritable_throwsMobileHarnessException()
      throws Exception {
    String filePathOnDevice = "/path/to/file";
    when(adb.runShellWithRetry(
            SERIAL,
  ...
  @Test
  public void remountSdk36Emulator() throws Exception {
    when(adb.run(eq(SERIAL), aryEq(new String[] {"shell", "remount", "/"})))
        .thenReturn("")
        .thenThrow(
            new MobileHarnessException(
  ...
  public void setFileSecurityContext_succeeds() throws Exception {
    ...
    when(adb.runShellWithRetry(SERIAL, "chcon -R %s %s".formatted(securityContext, filePath)))
