...
public final class CompiledDeviceInfoMaskTest {
  ...
  @Test
  public void of_deviceInfoMaskWithoutDeviceId_trimsDeviceIdOnBuild() {
    DeviceInfoMask mask =
        DeviceInfoMask.newBuilder()
    ...
    assertThat(compiled.deferredTrimMask()).isEmpty();
    DeviceInfo built =
        compiled
            .newMaskedDeviceInfoBuilder()
            .setFields(
            ...
            .build()
            .orElseThrow();
    assertThat(built.getDeviceLocator().getId()).isEmpty();
    assertThat(built.getDeviceStatus()).isEqualTo(DeviceStatus.IDLE);
  }

  @Test
  public void of_labQuery_enablesOnlyGroupBySubFieldGatesAndSetsDeferredTrimMask() {
    DeviceInfoMask rawDeviceInfoMask =
        DeviceInfoMask.newBuilder()
            .setFieldMask(FieldMask.newBuilder().addPaths("device_locator.id"))
            .build();
    LabQuery query =
        LabQuery.newBuilder()
            .setMask(Mask.newBuilder().setDeviceInfoMask(rawDeviceInfoMask))
                        ...
                        DeviceGroupOperation.newBuilder()
                            .setDeviceGroupCondition(
                                DeviceGroupCondition.newBuilder()
                                    .setTypeList(TypeList.getDefaultInstance()))))
            .build();
    CompiledDeviceInfoMask compiled = CompiledDeviceInfoMask.of(query);

    assertThat(compiled.deferredTrimMask()).hasValue(rawDeviceInfoMask);
    DeviceFeature projected = compiled.projectDeviceFeature(FULL_FEATURE);
