...
public final class CompiledDeviceInfoMaskTest {
  ...
  @Test
  public void trimFields_prunesToFieldMask() {
    DeviceInfo full = DeviceInfo.newBuilder().setDeviceFeature(FULL_FEATURE).build();

    DeviceInfoMask keepsFeature =
        DeviceInfoMask.newBuilder()
            .setFieldMask(FieldMask.newBuilder().addPaths("device_feature"))
            .build();
    assertThat(CompiledDeviceInfoMask.of(keepsFeature).trimFields(full).hasDeviceFeature())
        .isTrue();

    DeviceInfoMask dropsFeature =
        DeviceInfoMask.newBuilder()
            .setFieldMask(FieldMask.newBuilder().addPaths("device_condition"))
            .build();
    assertThat(CompiledDeviceInfoMask.of(dropsFeature).trimFields(full).hasDeviceFeature())
        .isFalse();
  }
}
