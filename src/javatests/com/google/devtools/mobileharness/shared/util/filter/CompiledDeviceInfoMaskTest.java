...
public final class CompiledDeviceInfoMaskTest {
  ...
  public void none_keepsEverything() {
    ...
    DeviceInfo info =
        mask.newMaskedDeviceInfoBuilder()
            .setDeviceLocator("d1", id -> DeviceLocator.newBuilder().setId(id).build())
            .setDeviceStatus("d1", id -> DeviceStatus.IDLE)
            .setMaskedDeviceFeature(FULL_FEATURE)
            .build();
  ...
  public void of_singleDimensionProjection_filtersDimensionsAndPreservesProperties() {
    ...
    DeviceInfo info =
        compiled
            ...
            .setDeviceLocator("d1", id -> DeviceLocator.newBuilder().setId(id).build())
            .setDeviceStatus("d1", id -> DeviceStatus.BUSY)
    ...
    assertThat(info.getDeviceLocator().getId()).isEqualTo("d1");
    // Device status was not requested in field_mask so it was skipped by setDeviceStatus.
