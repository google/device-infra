...
public final class CompiledLabInfoMaskTest {
  ...
  @Test
  public void of_labQuery_retainsHostNameForLabSortAndSetsDeferredTrimMask() {
    LabInfoMask rawLabInfoMask =
        LabInfoMask.newBuilder()
            .setFieldMask(FieldMask.newBuilder().addPaths("lab_status"))
            .build();
    LabQuery query =
        LabQuery.newBuilder().setMask(Mask.newBuilder().setLabInfoMask(rawLabInfoMask)).build();
    CompiledLabInfoMask compiled = CompiledLabInfoMask.of(query);

    assertThat(compiled.deferredTrimMask()).hasValue(rawLabInfoMask);
    LabInfo built =
        compiled
            .newMaskedLabInfoBuilder()
            .setFields(
