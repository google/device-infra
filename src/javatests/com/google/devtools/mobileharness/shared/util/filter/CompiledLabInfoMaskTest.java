...
public final class CompiledLabInfoMaskTest {
  ...
  public void none_keepsEverything() {
    ...
    LabInfo info =
        mask.newMaskedLabInfoBuilder()
            .setLabLocator(
                "host1", h -> LabLocator.newBuilder().setHostName(h).setIp("1.2.3.4").build())
            .setLabStatus("host1", h -> Optional.of(LabStatus.LAB_RUNNING))
            .build();
  ...
  public void of_selectiveProjection_prunesUnrequestedFieldsAndSkipsUnrequestedGetters() {
    ...
    boolean[] serverSettingCalled = new boolean[] {false};
    LabInfo info =
        compiled
            ...
            .setLabServerSetting(
                "host1",
                h -> {
                  serverSettingCalled[0] = true;
                  return Optional.empty();
                })
            .build();

    assertThat(serverSettingCalled[0]).isFalse();
    assertThat(info.getLabLocator().getHostName()).isEqualTo("host1");
