    ...
    com.google.devtools.mobileharness.infra.ats.console.result.proto.ReportProto.Result.Builder
        resultBuilder =
            com.google.devtools.mobileharness.infra.ats.console.result.proto.ReportProto.Result
                .newBuilder()
                .setSummary(
                    Summary.newBuilder().setPassed(5).setFailed(0).setModulesTotal(1).build());
    when(sessionResultHandlerUtil.processResult(
            any(), any(), any(), any(), eq(ImmutableList.of(jobInfo)), any()))
        .thenReturn(Optional.of(resultBuilder.build()));
    plugin.onSessionEnded(new SessionEndedEvent(sessionInfo, null));
  ...
  public void onTestMessage_testModuleResult_updateSessionOutput() throws Exception {
    ...
    XtsTradefedTestModuleResultsMessage resultsMessage =
        XtsTradefedTestModuleResultsMessage.newBuilder()
            .setTestModuleResultsString(results.encodeToString())
            .build();
    ImmutableMap<String, String> message =
        ImmutableMap.of(
            ...
            "textproto",
            TextFormat.printer().emittingSingleLine(true).printToString(resultsMessage));
