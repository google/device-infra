...
public class PlaywrightWebDriverTest {
  ...
  public void setUp() throws Exception {
    ...
    driver = new PlaywrightWebDriver(device, testInfo, cmdExecutor);
  ...
  public void run_success_generatesCorrectCommandAndEnv() throws Exception {
    ...
    // Verify test set to PASS
    verify(testResult).setPass();
    verify(subTestResult).setPass(true);
  }
  ...
  public void run_commandFailure_marksFailAndParsesPartialResults() throws Exception {
    ...
    verify(subTests).add("company#failed test");
    verify(subTestResult).setNonPassing(eq(TestResult.FAIL), any(MobileHarnessException.class));
  }
  ...
  public void run_interrupted_propagatesInterruptedButParsesResults() throws Exception {
    ...
    verify(subTests).add("company#partial test");
    verify(subTestResult).setPass(true);
    verify(testResult, Mockito.never()).setPass();
  }

  @Test
  public void getSeleniumAddress_returnsAddressFromParamsOrProperties() {
    // Case 1: Provided in jobInfo.params()
    when(params.get(PlaywrightWebDriver.PARAM_SELENIUM_ADDRESS, null))
        .thenReturn("http://params-address:4444/wd/hub");
    assertThat(driver.getSeleniumAddress(testInfo)).isEqualTo("http://params-address:4444/wd/hub");

    // Case 2: Not in params, but in testInfo.properties()
    ...
    assertThat(driver.getSeleniumAddress(testInfo))
        .isEqualTo("http://properties-address:4444/wd/hub");

    // Case 3: In neither
    when(properties.get(PlaywrightWebDriver.PARAM_SELENIUM_ADDRESS)).thenReturn(null);
    assertThat(driver.getSeleniumAddress(testInfo)).isNull();
  }
  ...
  public void run_missingXmlResults_logsWarning() throws Exception {
    ...
    // Verify subTests.add() was never called since there are no XML results to parse
    verify(subTests, Mockito.never()).add(any(String.class));
  }
  ...
  public void run_corruptXmlResults_logsWarning() throws Exception {
    ...
    // Verify subTests.add() was never called since parsing failed
    verify(subTests, Mockito.never()).add(any(String.class));
  }
