...
public class AndroidChromeDriverProviderDecoratorTest {
  ...
  public void setUp_withTestTimer_setsCommandTimeout() throws Exception {
    ...
    assertThat(executedCommand.getTimeout().isPresent()).isTrue();
    assertThat(executedCommand.getTimeout().get().getDeadline().isPresent()).isTrue();
    assertThat(executedCommand.getTimeout().get().getDeadline().get()).isEqualTo(mockTimer);
  }
}
