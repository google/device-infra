...
/** Unit tests for {@link PuppeteerWebDriver}. */
@RunWith(JUnit4.class)
public class PuppeteerWebDriverTest {
  ...
  private PuppeteerWebDriver driver;
  private File testFile;
  private File genFileDir;

  @Before
  public void setUp() throws Exception {
    testFile = tempFolder.newFile("puppeteer_test.sh");
    genFileDir = tempFolder.newFolder("genfiles");

    driver = new PuppeteerWebDriver(device, testInfo, cmdExecutor, localFileUtil, testXmlParser);

    when(testInfo.jobInfo()).thenReturn(jobInfo);
    when(jobInfo.files()).thenReturn(files);
    when(files.get(PuppeteerWebDriver.TAG_PUPPETEER_TEST_FILE))
  ...
  public void run_success_generatesCorrectCommandAndEnv() throws Exception {
    when(params.get(BaseWebTestDriver.PARAM_DEBUGGER_ADDRESS, null)).thenReturn(DEBUGGER_ADDRESS);
    when(params.get(BaseWebTestDriver.PARAM_BASE_URL, null)).thenReturn(BASE_URL);
    when(cmdExecutor.run(any(Command.class))).thenReturn("Puppeteer logs");
    ...
    assertThat(executedCommand.getCommand())
        .containsExactly(
            ...
            "--browserWSEndpoint=ws://" + DEBUGGER_ADDRESS,
    ...
    assertThat(executedCommand.getExtraEnvironment())
        .containsEntry("PUPPETEER_WS_ENDPOINT", "ws://" + DEBUGGER_ADDRESS);
  ...
  public void run_commandFailure_marksFail() throws Exception {
    ...
    when(commandException.getMessage()).thenReturn("Puppeteer exited with 1");
  ...
  @Test
  public void run_missingTestFile_throwsNoSuchElementException() {
    when(files.get(PuppeteerWebDriver.TAG_PUPPETEER_TEST_FILE)).thenReturn(ImmutableSet.of());

    assertThrows(NoSuchElementException.class, () -> driver.run(testInfo));
  }
}
