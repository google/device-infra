...
public final class AndroidTestLoopTestTest {
  ...
  public void setUp() throws Exception {
    ...
    when(resUtil.getExternalResourceFile(anyString())).thenReturn(Optional.empty());
  ...
  @Test
  public void run_partnerLabWithExternalResource_installsApk() throws Exception {
    AndroidTestLoopTestSpec spec =
        AndroidTestLoopTestSpec.newBuilder()
            .setAppPackageId("com.example.app")
            .setScenarios("1")
    ...
    when(resUtil.getExternalResourceFile(anyString()))
        .thenReturn(Optional.of("/path/to/external.apk"));
    setupInstrumentation((lineCallback) -> lineCallback.onLine("INSTRUMENTATION_CODE: -1"));
    ...
    verify(apkInstaller)
        .installApk(
            ...
            eq(
                ApkInstallArgs.builder()
                    .addApkPaths("/path/to/external.apk")
                    .setGrantPermissions(true)
                    .build()),
            any());
    verify(instrumentationUtil).instrument(eq(DEVICE_ID), any(), any(), any(), any());
