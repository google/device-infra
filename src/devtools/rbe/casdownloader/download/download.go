...
func (d *DownloadJob) downloadWithLocalCache(ctx context.Context, cache cache.Cache, outputs []*client.TreeOutput) error {
	...
	if len(toDownload) > 0 {
		...
		// Skip pushing if no new file is downloaded. This will save the time of initializing local
		// cache when cache lock is enabled.
		log.Infof("No file downloaded, skip pushing files to cache.")
	}
...
func (d *DownloadJob) doDownloadInternal(ctx context.Context) error {
	...
	start = time.Now()
	if err := chunkerutil.RestoreFiles(d.Dir, d.Dir); err != nil {
		return err
	}
