...
export function isAssistantEnabled(
  ...
  const queryParam =
    queryParamMap?.get('enable_assistant') ??
    (typeof window !== 'undefined'
      ? new URLSearchParams(window.location.search).get('enable_assistant')
      : null);
  if (queryParam === 'false') {
    try {
      window.localStorage?.setItem('enable_assistant', 'false');
    } catch {
  ...
  if (queryParam === 'true') {
    try {
      window.localStorage?.setItem('enable_assistant', 'true');
    } catch {
      // Ignore localStorage access errors.
    }
    return true;
