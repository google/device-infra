...
export class FakeTestService extends TestService {
   ...
   */
  override getTest(id: string): Observable<TestOverviewData> {
    const scenario = MOCK_TEST_SCENARIOS.find((s) => s.id === id || s.overview.id === id);
    if (scenario) {
  ...
  override getTestLog(
    ...
    if (scenario) {
      ...
      const chunk = visibleLog.substring(Number(offset), Number(offset) + length);
