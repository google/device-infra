...
describe('FakeAssistantService', () => {
  ...
  describe('submitActionDecision', () => {
    it('updates action status to APPROVED and persists to conversation record', (done) => {
      const convId = MOCK_CONVERSATION_HOST.conversationId;
      const turnId = MOCK_CONVERSATION_HOST.turns![0].turnId;
      const actionId =
        MOCK_CONVERSATION_HOST.turns![0].actionConfirmations![0].actionId;
      ...
      service
        ...
        .subscribe((res) => {
          ...
          expect(res.actionConfirmation.resolvedBy).toBe('operator@google.com');

          service.getConversation(convId).subscribe((getRes) => {
            const turn = getRes.conversation.turns![0];
            expect(turn.actionConfirmations![0].status).toBe(
              ActionDecisionStatus.APPROVED,
            );
            done();
          });
    ...
    it('streams actionConfirmation event when prompt requests a mutation', (done) => {
      const convId = MOCK_CONVERSATION_FLEET.conversationId;
      service
        .sendMessage({
          conversationId: convId,
          userPrompt: 'reboot pixel8-atl-01',
        })
        .subscribe((sendRes) => {
          const turnId = sendRes.turnId;
          // Poll batch 1
          service
            .pollEvents({conversationId: convId, turnId, lastEventSeq: 0})
            .subscribe((poll1) => {
              // Poll batch 2 (contains actionConfirmation at seq 4)
              service
                .pollEvents({
                  conversationId: convId,
                  turnId,
                  lastEventSeq: poll1.nextEventSeq,
                })
                .subscribe((poll2) => {
                  const hasActionConfirmation = poll2.events.some((e) =>
                    Boolean(e.actionConfirmation),
                  );
                  expect(hasActionConfirmation).toBeTrue();
                  done();
                });
            });
        });
