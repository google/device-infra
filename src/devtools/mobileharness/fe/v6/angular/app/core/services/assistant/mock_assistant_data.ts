  ...
  turns: [
    {
      ...
      actionConfirmations: [
        {
          actionId: 'action_host_002_drain',
          toolName: 'DrainHost',
          targetEntity: 'lab-host-05',
          summary:
            'Drain host lab-host-05 and gracefully complete active sessions before maintenance.',
          riskLevel: ActionRiskLevel.MEDIUM,
          status: ActionDecisionStatus.PENDING,
          parametersJson:
            '{"hostname": "lab-host-05", "graceful_drain_timeout_sec": 300}',
        },
      ],
    },
  ],
...
export function createSimulatedTurnEvents(
  ...
  const isMutationQuery =
    promptLower.includes('reboot') ||
    promptLower.includes('drain') ||
    promptLower.includes('release') ||
    promptLower.includes('restart');
    ...
    return [
      {
        ...
        thoughtDelta: {
          text: 'Evaluating requested operational mutation and safety preconditions...',
        },
      },
      {
        seq: 2,
        timestamp: new Date().toISOString(),
        thoughtDelta: {
          text: ' Preparing human-in-the-loop authorization request...',
        },
      },
      {
        seq: 3,
        ...
        messageDelta: {
          text:
            `I have prepared the operational action requested in **"${userPrompt}"**.\n\n` +
            'Please review and approve or reject the mutation below:',
        },
      },
      {
        seq: 4,
        timestamp: new Date().toISOString(),
        actionConfirmation: {
          actionId: `action_${turnId}_1`,
          toolName: 'RebootDevice',
          targetEntity: 'pixel8-atl-01',
          summary: 'Reboot device pixel8-atl-01 and clear ADB daemon cache.',
          riskLevel: ActionRiskLevel.HIGH,
          status: ActionDecisionStatus.PENDING,
          parametersJson:
            '{"device_id": "pixel8-atl-01", "mode": "HARD_REBOOT"}',
        },
      },
      {
        seq: 5,
