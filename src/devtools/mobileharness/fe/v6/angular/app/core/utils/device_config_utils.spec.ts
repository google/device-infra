...
describe('deviceConfigUtils', () => {
  ...
  it('should filter out dimensions with empty name but non-empty value', () => {
    const dimensions = {
      supported: [{name: '', value: 'val'}],
      required: [{name: '', value: 'val'}],
    };
    expect(clearEmptyDimensions(dimensions)).toEqual({
      supported: [],
      required: [],
    });
  });
