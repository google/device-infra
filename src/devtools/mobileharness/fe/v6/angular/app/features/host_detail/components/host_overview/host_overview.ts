...
export class HostOverviewPage {
  ...
  readonly displayLabType = computed<string | null>(() => {
    ...
    if (
      host.uiLabType &&
      host.uiLabType !== ('UI_LAB_TYPE_UNSPECIFIED' as UiLabType)
    ) {
      const mapped = this.mapUiLabTypeToString(host.uiLabType);
      if (mapped !== 'Unknown') {
        return mapped;
      }
    }
  ...
  readonly isSatelliteLab = computed(() => {
    const host = this.localHost();
    return (
      host.uiLabType === 'SATELLITE' ||
      (host.uiLabTypes?.includes('SATELLITE') ?? false)
    );
  });
