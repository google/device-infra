...
import {ConfigSection, type DeviceConfig} from '../../../../../core/models/device_config_models';
import {type Editability, type GetHostConfigResult, HostConfig, HostConfigSection, type UpdateHostConfigRequest} from '../../../../../core/models/host_config_models';
...
export class HostSettings implements OnInit {
      ...
      children:
          [
            ...
            },
            {
              ...
              icon: 'badge',
              label: 'Permissions',
              visibility: () => this.config.uiStatus.deviceConfig.visible,
            },
            {
              id: 'wifi',
              icon: 'wifi',
  ...
  // in the UI.
  dimensionsUiStatus = {
    sectionStatus: this.config.uiStatus.deviceConfig,
  };
  ...
  ngOnInit() {
    ...
    const tooltip = this.hostConfig.deviceConfigMode === 'PER_DEVICE' ?
  ...
  setActiveSectionEditibility(section: string) {
    switch (section) {
      ...
      case 'config-mode':
        this.activeSectionEditibility.set(
            this.config.uiStatus.deviceConfigMode.editability ||
                {editable: true},
        );
        break;
      case 'permissions':
      ...
      case 'stability':
        this.activeSectionEditibility.set(
            this.config.uiStatus.deviceConfig.editability || {editable: true},
        );
        break;
      case 'device-discovery':
        this.activeSectionEditibility.set(
            this.config.uiStatus.deviceDiscovery.editability ||
                {editable: true},
        );
        break;
  ...
  initializeVisibleSections() {
    ...
    if (firstVisibleSection.type === 'group') {
      const firstVisibleChild = firstVisibleSection.children?.find(
  ...
  save(selfLockout = false) {
    ...
    this.configService.updateHostConfig(request)
        .pipe(
            tap(() => {
