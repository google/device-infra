  ...
  implements OnInit, AfterViewInit, OnDestroy, OnChanges
{
  ...
  ngOnInit(): void {
    ...
    this.searchSubject
      ...
      .subscribe((searchTerm) => {
        ...
        console.log('filteredDimensions', filtered);
  ...
  private flattenDimensions(
    items: Record<string, DimensionSourceGroup>,
    section: 'supported' | 'required',
  ): void {
    const sectionItems = Object.entries(items).flatMap(([source, keyMap]) =>
      Object.entries(keyMap).flatMap(([key, values]) =>
        values.map((value) => {
          return {
            section,
            source,
    ...
    console.log('flatDimensions', this.flatDimensions);
  }
