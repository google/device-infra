...
export class AssistantPage implements OnInit {
    ...
    this.updateTurnById(turnId, (turn) => {
      ...
      for (const event of events) {
        ...
        if (event.actionConfirmation) {
          const action = event.actionConfirmation;
          const existingIdx = actionConfirmations.findIndex(
            (a) => a.actionId === action.actionId,
          );
          if (existingIdx >= 0) {
            actionConfirmations[existingIdx] = action;
          } else {
  ...
  onActionDecision(
    ...
    this.assistantService
      ...
      .subscribe({
        next: (res) => {
          this.updateTurnById(turn.turnId, (existingTurn) => ({
            ...existingTurn,
            actionConfirmations: (existingTurn.actionConfirmations ?? []).map(
              (item) =>
                item.actionId === action.actionId
                  ? res.actionConfirmation
                  : item,
            ),
          }));
        },
