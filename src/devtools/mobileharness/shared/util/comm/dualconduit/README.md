...
### Establishing a Conduit or a Session

-   **Conduit**: A low-level connection representing an individual transport
    link.
-   **Conduit Session**: A higher-level session that may manage multiple
    conduits simultaneously. It handles automatic reconnection and
    re-establishes conduits if they are closed or experience failures.
    **(Recommended: Conduit sessions are preferred over raw/low-level conduits
    for stability and resilience to network disconnects)**.

Once both Acceptor and Dialer are running, you can establish a conduit or a
session:

#### 1. Via Command Line Flag (`-L`) on Dialer Startup to establish forward conduit sessions

You can specify conduit sessions to be established immediately when starting the
dialer using the `-L` flag.
...
manage conduits and sessions dynamically.

You can use a gRPC client to call `EstablishConduit` or `EstablishSession` on
the Dialer (listening on port `50051` in the example above). We recommend using
`EstablishSession` to set up persistent, auto-reconnecting connections.
...
> `EstablishConduit` or `EstablishSession` gRPC method on the Dialer. They
> cannot be established via the `-L` command-line flag.
...
## Tearing Down a Conduit or a Session

To tear down an established conduit, you can use the `TeardownConduit` gRPC
method on the Dialer service by providing the `conduit_id` returned in the
`EstablishConduitResponse`.

To tear down a conduit session, you can use the `TeardownSession` gRPC
method on the Dialer service. You need to provide the `session_id` which was
returned in the `EstablishSessionResponse` when the session was created.
