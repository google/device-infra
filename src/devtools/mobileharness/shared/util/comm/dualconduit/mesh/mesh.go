...
func (s *Server) generateSnapshot(version string) (*cache.Snapshot, error) {
	...
	for logicalName, info := range s.services {
		...
		if info.Protocol == "tcp" {
			...
			// The use of FilterChainMatch with ServerNames means that these TCP services
			// are expected to be fronted by TLS, with the client providing the logicalName
			// as the Server Name Indication (SNI). Raw TCP traffic without TLS will not
			// be matched by these filter chains.
