...
static inline void markEvent() { g_lastEventMs = millis(); }
...
void logMsg(const char* format, ...) {
  ...
  // Wrap around / Clear screen if the next line goes off the bottom
  if (M5.Display.getCursorY() + (8 * M5.Display.getTextSizeY()) >
      M5.Display.height()) {
    showLogScreenHeader();
  }
...
void releaseServo(uint8_t boardAddr, uint8_t channel) {
  if (channel >= MAX_CHANNELS) {
    logMsg("ERR: invalid channel");
    return;
  }
  int index = getBoardIndex(boardAddr);
  if (index < 0) {
    logMsg("ERR: board not found");
    return;
  }
...
void pressServo(uint8_t boardAddr, uint8_t channel, uint32_t duration) {
  ...
  logMsg("OK,PRESS,0x%02X,ch=%u,ms=%lu,tick=%u", boardAddr, channel, duration,
         pressT);
}
  ...
  for (int b = 0; b < (int)numBoards; ++b) {
    for (int c = 0; c < MAX_CHANNELS; ++c) {
      if (servoStates[b][c].active) {
        ...
        logMsg(" - 0x%02X ch%d remaining=%d ms", boardAddresses[b], c, remain);
      }
    }
  }
  logMsg("=============");
...
void cmd_press_ms(int argc, char** argv) {
  if (argc < 4) {
    logMsg("ERR: press_ms <boardHex> <ch> <ms>");
    return;
  }
  uint8_t board =
      static_cast<uint8_t>(strtoul(argv[1], nullptr, 16));             // NOLINT
...
void cmd_release(int argc, char** argv) {
  if (argc < 3) {
    logMsg("ERR: release <boardHex> <ch>");
    return;
  }
  uint8_t board =
      static_cast<uint8_t>(strtoul(argv[1], nullptr, 16));           // NOLINT
...
void cmd_raw_tick(int argc, char** argv) {
  if (argc < 4) {
    logMsg("ERR: raw_tick <boardHex> <ch> <tick>");
    return;
  }
  uint8_t board =
      static_cast<uint8_t>(strtoul(argv[1], nullptr, 16));           // NOLINT
  uint8_t ch = static_cast<uint8_t>(strtoul(argv[2], nullptr, 10));  // NOLINT
  uint16_t tick =
      static_cast<uint16_t>(strtoul(argv[3], nullptr, 10));  // NOLINT
...
void cmd_cal_set(int argc, char** argv) {
  ...
  uint16_t pressT =
      static_cast<uint16_t>(strtoul(argv[3], nullptr, 10));  // NOLINT
  uint16_t relT =
      static_cast<uint16_t>(strtoul(argv[4], nullptr, 10));  // NOLINT
...
void cmd_cal_get(int argc, char** argv) {
  if (argc < 3) {
    logMsg("ERR: cal_get <boardHex> <ch>");
    return;
  }
  uint8_t board =
      static_cast<uint8_t>(strtoul(argv[1], nullptr, 16));           // NOLINT
  ...
  logMsg("OK,CAL_GET,0x%02X,ch=%u,press=%u,release=%u,source=%s", board, ch,
         pressT, relT, g_cal[bi][ch].has ? "calibrated" : "default");
}

void cmd_cal_reset(int argc, char** argv) {
  if (argc < 3) {
    logMsg("ERR: cal_reset <boardHex> <ch>");
    return;
  }
  uint8_t board =
      static_cast<uint8_t>(strtoul(argv[1], nullptr, 16));           // NOLINT
...
void dispatchCommand(char* line) {
  ...
  // Ignore echoed logs or system messages to prevent infinite Serial ECHO loops
  if (strncmp(argv[0], "AUTO_RELEASE", 12) == 0 ||
      strncmp(argv[0], "auto", 4) == 0 || strncmp(argv[0], "ERR:", 4) == 0 ||
      strncmp(argv[0], "OK,", 3) == 0 || strncmp(argv[0], "This", 4) == 0 ||
      strncmp(argv[0], "System", 6) == 0 || strncmp(argv[0], "Found", 5) == 0 ||
      strncmp(argv[0], "RecoverBot", 10) == 0) {
    return;
  }
