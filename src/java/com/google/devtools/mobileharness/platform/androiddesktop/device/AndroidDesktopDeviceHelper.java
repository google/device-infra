...
public class AndroidDesktopDeviceHelper {
  ...
  public void updateDeviceDutState(String deviceId, String newState)
      throws MobileHarnessException, InterruptedException {
    updateDeviceDutState(
        deviceId,
        newState,
        /* provision= */ false,
        /* reimage= */ false,
  ...
  public void updateDeviceDutState(
      String deviceId,
      String newState,
      boolean provision,
      boolean reimage,
      boolean usbkey,
      boolean clearRepairRequests)
      throws MobileHarnessException, InterruptedException {
    if (isNullOrEmpty(newState) && !provision && !reimage && !usbkey && !clearRepairRequests) {
      throw new MobileHarnessException(
          ExtErrorId.ANDROID_DESKTOP_DEVICE_HELPER_INFRA_ERROR,
          "At least one update flag (newState, provision, reimage, usbkey, clearRepairRequests)"
              + " must be provided");
    }
    if (clearRepairRequests && (provision || reimage || usbkey)) {
      throw new MobileHarnessException(
          ExtErrorId.ANDROID_DESKTOP_DEVICE_HELPER_INFRA_ERROR,
          "Cannot set repair requests with clearRepairRequests");
    }
    String hostName = getHostName(deviceId);
    List<String> commandArgs = new ArrayList<>();
    commandArgs.add(SHIVAS_CLI_PATH);
    commandArgs.add("update");
    commandArgs.add("dut-state");
    if (!isNullOrEmpty(newState)) {
      commandArgs.add("-dut-state");
      commandArgs.add(newState);
    }
    commandArgs.add("-dut-name");
    commandArgs.add(hostName);
    if (provision) {
      commandArgs.add("-provision");
    }
    if (reimage) {
      commandArgs.add("-reimage");
    }
    if (usbkey) {
      commandArgs.add("-usbkey");
    }
    if (clearRepairRequests) {
      commandArgs.add("-clear-repair-requests");
    }
    commandArgs.add("-service-account-json");
    commandArgs.add(SA_PATH);
    Command command = Command.of(commandArgs);
    logger.atInfo().log("shivas command: %s", command);
    CommandResult r = commandExecutor.exec(command);
    if (r.exitCode() == 0) {
      logger.atInfo().log("Successfully updated device %s dut state", hostName);
    } else {
      logger.atWarning().log(
          "Failed to update device %s dut state, exit code: %s, stdout: %s, stderr: %s",
          deviceId, r.exitCode(), r.stdout(), r.stderr());
      throw new MobileHarnessException(
          ExtErrorId.ANDROID_DESKTOP_DEVICE_HELPER_INFRA_ERROR,
          String.format(
              "Failed to update device %s dut state, stdout: %s, stderr: %s",
              hostName, r.stdout(), r.stderr()));
    }
  }
