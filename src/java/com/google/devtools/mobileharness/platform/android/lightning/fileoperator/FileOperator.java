...
public class FileOperator {
  ...
  public boolean pull(String serial, FilePullArgs pullArgs, @Nullable LogCollector<?> log)
    ...
    try {
      ...
      SharedLogUtil.logMsg(
          logger,
          String.format("Pulled file/dir %s from device %s:%n%s", srcPathOnDevice, serial, output),
          log);
      ...
      if (logFailuresOnly) {
        String msg =
            String.format(
                ...
                srcPathOnDevice,
                serial);
        SharedLogUtil.logMsg(logger, msg, Level.WARNING, log, e);
      } else {
  ...
  public void pushFileOrDir(Device device, FilePushArgs pushArgs, @Nullable LogCollector<?> log)
    ...
    // Actual push.
    SharedLogUtil.logMsg(
        logger, String.format("Start pushing file/dir to device %s:%n%s", deviceId, info), log);
    try {
      String output =
      ...
      if (!androidFileUtil.isFileOrDirExisted(deviceId, fileOrDirPath)) {
        SharedLogUtil.logMsg(
            ...
            String.format(
                ...
                fileOrDirPath, deviceId),
            log);
      } else {
        androidFileUtil.removeFiles(deviceId, fileOrDirPath);
        SharedLogUtil.logMsg(
            logger,
            String.format("Removed file/dir [%s] in device %s", fileOrDirPath, deviceId),
            log);
      }
    } catch (MobileHarnessException e) {
  ...
  private void prepareDir(Device device, String fileOrDirPath, @Nullable LogCollector<?> log)
    ...
    if (androidFileUtil.isDirExist(deviceId, sdkVersion, dirPath)) {
      SharedLogUtil.logMsg(
          logger,
          String.format("Destination dir %s already existed in device %s", dirPath, deviceId),
          log);
      return;
    }
    try {
      ...
      SharedLogUtil.logMsg(
          logger,
          String.format(
              "Creates dir %s for destination %s in device %s", dirPath, fileOrDirPath, deviceId),
          log);
    } catch (MobileHarnessException e) {
      SharedLogUtil.logMsg(
          logger,
          String.format(
              ...
              dirPath, fileOrDirPath, deviceId),
          log);
      throw new MobileHarnessException(
