...
public class InventoryClient implements AutoCloseable {
  ...
  /**
   * Creates a connection to the InventoryService.
   *
   * @param serverHost The hostname or IP address of the InventoryService server.
   * @param serverPort The port number of the InventoryService server.
   */
  ...
  public List<Dut> getDuts(String dutName) throws InterruptedException {
    if (asyncStub == null) {
      throw new IllegalStateException("InventoryClient is not connected.");
    }
    ...
    asyncStub.withDeadlineAfter(Duration.ofSeconds(10)).getDutTopology(request, responseObserver);
    latch.await();
    return dutsRef.get();
  }
  ...
  public static void main(String[] args) throws InterruptedException {
    try (InventoryClient client = new InventoryClient()) {
      String host = "localhost"; // Replace with your server host
      int port = 1485; // Replace with your server port
      String dutTopologyId = "chromeos8-row1-rack2-host4"; // Replace with the desired id
