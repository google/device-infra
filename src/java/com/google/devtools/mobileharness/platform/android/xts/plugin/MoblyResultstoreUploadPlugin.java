  ...
  public void onTestEnding(TestEndingEvent event) throws InterruptedException {
    ...
    try {
      ...
      try {
        ...
        if (!devices.isEmpty()) {
          String firstDeviceSerial = devices.get(0).getSerial();
          fingerprint =
              androidAdbUtil.getProperty(
                  firstDeviceSerial, ImmutableList.of("ro.build.fingerprint"));
        }
      } catch (MobileHarnessException e) {
        testInfo
            .log()
            .atWarning()
            .alsoTo(logger)
            .log("Failed to get device build fingerprint: %s", e.getMessage());
      }
      if (fingerprint != null && !fingerprint.trim().isEmpty()) {
        uploadCmd = uploadCmd.argsAppended("--label", fingerprint.trim());
      }
      try {
