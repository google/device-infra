...
/** Lab plugin to upload test gen files to ATS2.0 file server. */
      ...
      try {
        ...
        if (e instanceof CommandFailureException) {
          ...
          logger.atWarning().log(
              "Logs of failed http uploader: STDOUT: %s\nSTDERR: %s",
              result.stdout(), result.stderr());
        }
        throw new MobileHarnessException(
            BasicErrorId.HTTP_UPLOAD_FILE_ERROR,
            String.format("Failed to upload file %s to %s in http server.", genFile, atsFileServer),
            e);
      }
    }
  }
