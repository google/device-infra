...
/** Implementation of {@link TestManager}. */
public class TestManagerImpl<T extends TestRunner> extends AbstractScheduledService
    implements TestManager<T> {
  ...
  @VisibleForTesting
  TestManagerImpl(SystemUtil systemUtil, Sleeper sleeper) {
  ...
  public Scheduler scheduler() {
    return Scheduler.newFixedDelaySchedule(Duration.ZERO, CHECK_TEST_INTERVAL);
  }
  ...
  void logAllStackTraces() {
    logger.atWarning().atMostEvery(5, MINUTES).log(
        "%s", lazy(TestManagerImpl::formatAllStackTraces));
  }
  ...
  abstract static class ZombieTestInfo {
    private static ZombieTestInfo create(TestRunner testRunner, int killCount) {
      return new AutoValue_TestManagerImpl_ZombieTestInfo(testRunner, killCount);
    }
