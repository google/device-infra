...
public class TestRetryHandler {
  ...
  private RetryStrategy getRetryStrategy(JobInfo jobInfo) {
    String flakyTestAttemptsStr = jobInfo.params().get("flaky_test_attempts");
    if (flakyTestAttemptsStr != null) {
      try {
        if (Integer.parseInt(flakyTestAttemptsStr) != 0) {
          jobInfo
              .log()
              .atInfo()
              .alsoTo(logger)
              .log(
                  "Using FlakyTestRetryStrategy. All retry_level/test_attempts args will be"
                      + " ignored.");
