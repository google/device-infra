...
class LocalDeviceManagerSchedulerSyncer implements ApiConfigListener {
  ...
  LocalDeviceManagerSchedulerSyncer(
    ...
    deviceTempRequiredDimensionManager.setListener(this::onDeviceTempRequiredDimensionChanged);
  }
  ...
  @VisibleForTesting
  void onDeviceTempRequiredDimensionChanged(
      DeviceTempRequiredDimensionManager.DeviceKey deviceKey) {
    String deviceUuid = deviceKey.deviceUuid();
    LocalDeviceRunner deviceRunner = deviceManager.getLocalDeviceRunner(deviceUuid);
    if (deviceRunner != null) {
      DeviceStatus deviceStatus = deviceRunner.getDeviceStatus();
      logger.atInfo().log(
          "Update device %s to scheduler due to temp required dimension change", deviceUuid);
      upsertDeviceToScheduler(deviceStatus, deviceUuid, /* deviceType= */ null, deviceRunner);
    } else {
      logger.atInfo().log("Device %s not found for temp required dimension change", deviceUuid);
    }
  }
    ...
    deviceTempRequiredDimensionManager
        ...
        .ifPresent(
            dimensions ->
                dimensions
                    .dimensions()
                    .forEach((name, value) -> deviceUnit.dimensions().required().add(name, value)));
