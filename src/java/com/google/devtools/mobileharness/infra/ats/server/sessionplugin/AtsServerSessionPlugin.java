    ...
    synchronized (sessionLock) {
      try {
        ...
        handleResultProcessingResult
            .errorMessage()
            .ifPresent(errorMessage -> appendErrorMessage(requestDetail, errorMessage));
        requestDetail.putAllCommandDetails(handleResultProcessingResult.commandDetails());
        requestDetail.putAllTestContext(handleResultProcessingResult.testContexts());
      } catch (Throwable e) {
        requestDetail.setState(RequestState.ERROR);
        requestDetail.setErrorReason(ErrorReason.RESULT_PROCESSING_ERROR);
        requestDetail.setErrorMessage(e.getMessage());
        throw e;
      } finally {
        updateSessionPluginOutput();
      }
