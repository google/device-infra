...
final class NewMultiCommandRequestHandler {
  ...
  /**
   * Waits for all devices specified in the {@code commandInfo} to become idle with best effort.
   *
   * <p>This method repeatedly queries for device statuses until all requested device serials are
   * reported as IDLE. It checks device status every {@link
   * #GET_DEVICE_FOR_ATS_SERVER_RETRY_INTERVAL}. The total wait time is determined by {@code
   * device_recovery_timeout} in {@code TestEnvironment}, defaulting to 20 minutes if not specified.
   *
   * <p>If any of the requested devices are still not idle after all retry attempts, this method
   * will log a warning but will not throw an exception. This method is best effort and not a
   * blocker to job creation.
   *
   * @param commandInfo Information about the command, including the list of requested device
   *     serials in device_dimension field.
   ...
   */
  void waitForRequestedDevicesToBeReady(
    ...
    for (DeviceDimension entry : commandInfo.getDeviceDimensionsList()) {
      if (entry.getName().equals("device_serial")) {
        requestedSerials.add(entry.getValue());
      }
    }
    ...
    int maxRetryAttempts = GET_DEVICE_FOR_ATS_SERVER_MAX_RETRY_ATTEMPTS;
    if (request.getTestEnvironment().hasDeviceRecoveryTimeout()) {
      maxRetryAttempts =
          (int)
              (request.getTestEnvironment().getDeviceRecoveryTimeout().getSeconds()
                  / GET_DEVICE_FOR_ATS_SERVER_RETRY_INTERVAL.toSeconds());
    }

    Set<String> missingSerials = new HashSet<>(requestedSerials);
    int attempt = 0;
    while (!missingSerials.isEmpty() && attempt < maxRetryAttempts) {
      ImmutableList<com.google.wireless.qa.mobileharness.shared.proto.query.DeviceQuery.DeviceInfo>
          deviceInfos;
      try {
        deviceInfos = atsMasterUtil.queryAndroidDevicesFromMaster();
        logger.atInfo().log("Device infos: %s", deviceInfos);
      } catch (MobileHarnessException e) {
        logger.atWarning().withCause(e).log(
            "Failed to query device during waitForRequestedDevicesToBeReady.");
        deviceInfos = ImmutableList.of(); // Continue with an empty result.
      }
      ImmutableSet<String> currentDevices =
          deviceInfos.stream()
              .filter(d -> d.getStatus().equals("IDLE"))
              .map(d -> d.getId())
              .collect(toImmutableSet());

      missingSerials.clear();
      missingSerials.addAll(requestedSerials);
      missingSerials.removeAll(currentDevices);

      if (!missingSerials.isEmpty()) {
        attempt++;
        if (attempt < maxRetryAttempts) {
          logger.atWarning().log(
              "Devices %s not idle, retry attempt %d/%d in %s...",
              missingSerials, attempt, maxRetryAttempts, GET_DEVICE_FOR_ATS_SERVER_RETRY_INTERVAL);
    ...
    if (!missingSerials.isEmpty()) {
      logger.atWarning().log(
          "Devices %s are still not idle after %d attempts.", missingSerials, maxRetryAttempts);
    }
  }
  ...
  private SessionRequestInfo generateSessionRequestInfo(
    ...
    if (deviceInfo.isEmpty()) {
      throw MobileHarnessExceptionFactory.createUserFacingException(
          InfraErrorId.ATS_SERVER_GET_DEVICE_INFO_ERROR,
          String.format(
              "Failed to get device info from devices: %s. This is likely due to device not in"
                  + " healthy state. You may extend device recovery timeout to give devices more"
                  + " time to recover.",
              String.join(",", deviceSerials)),
          /* cause= */ null);
    }
    sessionRequestInfoBuilder.setDeviceInfo(deviceInfo);

    sessionRequestInfoBuilder
        .setAtsServerTestResources(fileTestResources.build())
        .setAtsServerTestEnvironment(request.getTestEnvironment());
    return sessionRequestHandlerUtil.addXtsModuleInfo(sessionRequestInfoBuilder.build());
  }
