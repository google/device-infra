      ...
      if (outputUrl.getProtocol().equals("file")) {
        for (CommandDetail commandDetail : requestDetail.getCommandDetailsMap().values()) {
          ...
          if (localFileUtil.isFileExist(resultZip)) {
            TestContext.Builder testContextBuilder = TestContext.newBuilder();
            testContextBuilder.setCommandLine(commandDetail.getCommandLine());
            testContextBuilder.putAllEnvVars(
                requestDetail.getOriginalRequest().getTestEnvironment().getEnvVarsMap());
            testContextBuilder.addTestResources(
                TestResource.newBuilder()
                    .setName(resultZip.getFileName().toString())
                    .setUrl("file://" + resultZip.toString())
                    .build());
            requestDetail.putTestContexts(commandId, testContextBuilder.build());
          }
          try {
