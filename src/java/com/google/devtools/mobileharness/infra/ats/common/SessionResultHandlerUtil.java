...
public class SessionResultHandlerUtil {
   ...
   *
   * For ATS UI:
   * .../<session_id>/<command_id>/logs/
   *
   * Common structure under the log root:
   *    inv_<invocation_id>/
   *      raw tradefed logs
   *      <driver>_test_<test_id>/
   *        command_history.txt
   *        xts_tf_output.log
   *        tool-logs/ (filtered for ATS UI)
   *          ...
   *    non-tradefed_logs/
   ...
   * </pre>
   *
   * <p>For ATS UI requests, this method filters out redundant files generated by ClusterLogSaver
   * (e.g., large XML configs) in the "tool-logs" directory to save space.
   */
  @VisibleForTesting
  void copyTradefedTestLogFiles(TestInfo tradefedTestInfo, Path logRootDir, boolean isAtsUiRequest)
      throws MobileHarnessException, InterruptedException {
    ...
    for (Path genFile : genFiles) {
      ...
      } else if (isAtsUiRequest && genFile.getFileName().toString().equals("tool-logs")) {
        // If it's tool-logs dir, copy its content into the new log dir but skip large redundant
        // xml files for ATS UI.
        Path targetToolLogsDir = testLogDir.resolve("tool-logs");
        localFileUtil.prepareDir(targetToolLogsDir);
        List<Path> toolLogFilesOrDirs = localFileUtil.listFilesOrDirs(genFile, path -> true);
        for (Path toolLogFileOrDir : toolLogFilesOrDirs) {
          String fileName = toolLogFileOrDir.getFileName().toString();
          if (isRedundantToolLog(fileName)) {
            logger.atInfo().log(
                "Skipping redundant tradefed test log relevant file [%s] generated by"
                    + " ClusterLogSaver",
                toolLogFileOrDir);
            continue;
          }
          localFileUtil.copyFileOrDir(toolLogFileOrDir, targetToolLogsDir);
        }
  ...
  private boolean isRedundantToolLog(String fileName) {
    return fileName.startsWith("module-configuration")
        || fileName.startsWith("tradefed-expanded-config")
        || fileName.startsWith("subprocess-module-configuration")
        || fileName.startsWith("subprocess-tradefed-expanded-config")
        || fileName.startsWith("subprocess-device_logcat")
        || fileName.startsWith("subprocess-device_manifest")
        || fileName.startsWith("subprocess-framework_manifest")
        || (fileName.startsWith("subprocess-") && fileName.contains("DeviceInfo.deviceinfo.json"));
  }
  ...
  /** Returns the parameterized non-tradefed module name of the given non-tradefed job info. */
