...
public final class CompiledLabInfoMask {
  ...
  public static CompiledLabInfoMask of(LabQuery query) {
    ...
    if (!query.hasMask() || !query.getMask().hasLabInfoMask()) {
      return RETAIN_ALL;
    }
  ...
  private CompiledLabInfoMask(LabInfoMask mask, boolean hasDeviceViewRequest) {
    ...
    if (!keepsLabInfo || retainAllFields) {
      this.fieldMask = retainAllFields ? null : rawMask;
      this.needsPostSortTrim = false;
    } else if (!hasDeviceViewRequest
        && !MaskUtils.isFieldRequested(rawMask, "lab_locator.host_name")) {
      this.fieldMask = rawMask.toBuilder().addPaths("lab_locator.host_name").build();
      this.needsPostSortTrim = true;
    } else {
  ...
  /**
   * Whether a temporary sort field was added to the push-down mask that must be trimmed after
   * sorting completes.
   */
  ...
  public static final class MaskedLabInfoBuilder {
    ...
    public <S> MaskedLabInfoBuilder setMaskedLabServerSetting(
        S source, Function<S, LabServerSetting> getter) {
      if (mask.keepsLabServerSetting) {
        LabServerSetting setting = getter.apply(source);
        if (setting != null) {
    ...
    public <S> MaskedLabInfoBuilder setMaskedLabServerFeature(
        S source, Function<S, LabServerFeature> getter) {
      if (mask.keepsLabServerFeature) {
        LabServerFeature feature = getter.apply(source);
        if (feature != null) {
