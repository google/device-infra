...
public final class CompiledFieldMask {
  ...
  // A null path set means "no field mask was set", i.e. retain everything.
  private static final CompiledFieldMask RETAIN_ALL = new CompiledFieldMask(null, null);
  ...
  @Nullable private final ImmutableSet<String> paths;

  private CompiledFieldMask(
      @Nullable FieldMask rawFieldMask, @Nullable ImmutableSet<String> paths) {
  ...
  /**
   * Compiles the given field mask.
   *
   * <p>A {@code null} mask retains everything. An empty mask (present but with no paths) retains
   * nothing, which for a message-typed field means the message is dropped.
   */
  ...
  /** Returns the raw {@link FieldMask}, or {@code null} if all fields are retained. */
  @Nullable
  ...
  public NodeState stateOf(String field) {
    if (paths == null || paths.contains(field)) {
   ...
   *
   * <p>Returns {@code null} for a {@link NodeState#FULL} or {@link NodeState#OMITTED} child, where
   * no sub-mask trimming applies.
   */
  @Nullable
  public FieldMask subMask(String field) {
    if (paths == null || paths.contains(field)) {
      return null;
    }
    String prefix = field + ".";
    ...
    return builder.getPathsCount() == 0 ? null : builder.build();
  }
   ...
   * empty or {@code null}.
   */
  public static <M extends Message> M trimSubMessage(@Nullable FieldMask subMask, M message) {
    if (subMask == null || subMask.getPathsCount() == 0) {
      return message;
    }
  ...
  public <M extends Message> M trimIfUnknownPaths(boolean hasUnknownPaths, M message) {
    if (hasUnknownPaths && rawFieldMask != null) {
      return FieldMaskUtil.trim(rawFieldMask, message);
    }
    return message;
  }
}
