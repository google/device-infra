...
public final class JobInfoCreator {
  ...
  /** Creates JobInfo from Gateway's JobConfig. */
  public static JobInfo createJobInfo(
    ...
    if (jobConfig.hasRetry()) {
      ...
      // exist, so we need to manually set it to avoid inconsistence.
  ...
  /** Creates JobInfo from Gateway's JobConfig which does not contain MH's JobConfig. */
  private static JobInfo createJobInfo(
    ...
    JobInfo jobInfo =
        JobInfo.newBuilder()
            ...
            .build();
    for (StrPair dimension : jobConfig.getDimensionList()) {
      jobInfo.dimensions().add(dimension.getName(), dimension.getValue());
    }
    for (StrPair param : jobConfig.getParamList()) {
      jobInfo.params().add(param.getName(), param.getValue());
    }
    ...
    try {
      for (StrPair file : jobConfig.getFileList()) {
        ...
        if (localFileUtil.isLocalFileOrDir(fileValue)
          ...
          fileValue =
              PathUtil.join(jobSetting.getRunFileDir(), fileValue.replace(sessionTmpDir, ""));
    ...
    jobInfo
        .protoSpec()
        .setProto(
            jobInfo.protoSpec().getProto().toBuilder().mergeFrom(jobConfig.getJobSpec()).build());
      ...
      for (int i = 0; i < jobConfig.getDevice().getSubDeviceSpecCount(); i++) {
        ...
        for (Driver decorator : deviceSpec.getDecorators().getContentList()) {
          ...
          if (decorator.hasParam()) {
            try {
              Pair<String, JsonObject> scopedSpec =
                  getNamespaceAndScopedSpecs(decorator, /* isDecorator */ true);
              decoratorSpecs.put(scopedSpec.getFirst(), scopedSpec.getSecond());
            } catch (MobileHarnessException e) {
              throw new MobileHarnessException(
  ...
  @VisibleForTesting
  static Pair<String, JsonObject> getNamespaceAndScopedSpecs(Driver driver, boolean isDecorator)
      throws MobileHarnessException {
    ...
    return Pair.of(namespace, jsonParams.getAsJsonObject());
  }
  ...
  private static void addSpecsOfDriver(ScopedSpecs scopedSpecs, Driver driver, boolean isDecorator)
    ...
    try {
      Pair<String, JsonObject> scopedSpec = getNamespaceAndScopedSpecs(driver, isDecorator);
      scopedSpecs.add(scopedSpec.getFirst(), scopedSpec.getSecond());
    } catch (MobileHarnessException e) {
      throw new MobileHarnessException(
          BasicErrorId.JOB_SPEC_INVALID_JOB_TYPE_ERROR,
          String.format(
