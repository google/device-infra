...
public final class RelayInterceptor implements ServerInterceptor {
  ...
  public RelayInterceptor(ChannelManager channelManager) {
  ...
  public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
    ...
    String channelName = DestinationUtils.getDestination(headers);
    if (channelName == null) {
      logger.atInfo().log("No relay. Call the original service");
      return next.startCall(call, headers);
    } else {
      logger.atInfo().log("Relay to channel %s", channelName);
      return new RelayHandler<ReqT, RespT, ReqT, RespT>(
              channelManager, identity(), identity(), identity())
