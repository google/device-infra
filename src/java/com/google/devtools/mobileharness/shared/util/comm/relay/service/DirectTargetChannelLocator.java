...
public final class DirectTargetChannelLocator implements ChannelLocator {
  ...
  @Override
  public <T> T createStub(Destination destination, Function<Channel, T> stubCreator) {
    String grpcTarget = DestinationUtils.getTarget(destination);
    return channelManager.createStub(
        grpcTarget, () -> managedChannelSupplier.apply(grpcTarget), stubCreator);
  }
}
