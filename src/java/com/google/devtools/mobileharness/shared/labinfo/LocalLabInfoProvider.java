        ...
        devices.entrySet().stream()
            .map(
                entry -> {
                  ...
                  if (deviceMask.keepsDeviceLocator()) {
                    builder.setDeviceLocator(
                        deviceMask.trimDeviceLocator(
                            DeviceLocator.newBuilder().setId(device.getDeviceUuid()).build()));
                  }
                  if (deviceMask.keepsDeviceStatus()) {
                              ...
                              (name, value) ->
                                  conditionBuilder.addTempDimension(
                                      TempDimension.newBuilder()
                                          ...
                                          .setRequired(true)));
                      builder.setDeviceCondition(
                          deviceMask.trimDeviceCondition(conditionBuilder.build()));
                    }
                  }
                  if (deviceMask.keepsDeviceFeature()) {
                    ...
                    DeviceFeature feature = deviceMask.projectDeviceFeature(fullFeature);
                    if (feature != null) {
                      builder.setDeviceFeature(feature);
                    }
                  }
                  return deviceMask.build(builder);
                })
