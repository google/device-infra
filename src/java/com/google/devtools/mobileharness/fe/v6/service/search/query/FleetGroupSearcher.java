...
public final class FleetGroupSearcher {
   ...
   * by the flat searcher, so paging, sorting, and cell typing are identical to the flat view. The
   * page size is fixed at {@value #EXPAND_PAGE_SIZE}; the request supplies only a cursor.
   *
   * @param snapshot the fleet snapshot to search
   ...
   * @param sort the row sort order; when null or with an empty key, sorts by device UUID ascending
  ...
  public FleetFlatResults expandGroup(
    ...
    return flatSearcher.searchFlat(snapshot, filters, columnKeys, sort, page);
  }
