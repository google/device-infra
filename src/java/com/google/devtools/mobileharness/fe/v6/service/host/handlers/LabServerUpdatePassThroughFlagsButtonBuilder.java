...
/**
 * Utility class to build {@link ActionButtonState} for lab server update pass through flags button.
 */
@Singleton
public class LabServerUpdatePassThroughFlagsButtonBuilder {
  ...
  public ActionButtonState build(UniverseScope universe) {
    if (!featureManagerFactory.create(universe).isLabServerUpdatePassThroughFlagsFeatureEnabled()) {
      return ActionButtonState.newBuilder().setVisible(false).build();
    }

    return ActionButtonState.newBuilder()
        .setVisible(false)
        .setIsReady(featureReadiness.isLabServerUpdatePassThroughFlagsReady())
        .setEnabled(true)
        .setTooltip("Update pass through flags")
        .build();
  }
}
