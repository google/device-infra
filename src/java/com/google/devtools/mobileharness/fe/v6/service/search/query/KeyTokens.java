 ...
 * should be compared against alias tables and key ids? It knows nothing about which keys exist or
...
public final class KeyTokens {
  ...
  /** {@code dimension:<name>}, {@code dimension <name>}, {@code device_dimension:<name>}. */
  ...
  private static final Pattern HOST_PROPERTY_NAMESPACE =
      Pattern.compile("^host[ _]?property[ _:]+(.+)$");

  /**
   * Canonical form of a key token: trimmed, lowercased, with every run of whitespace or underscores
   * collapsed to a single underscore. This is the form alias tables are keyed by and the form a
   * bare token is compared against index key names.
   */
  public static String normalize(String token) {
    return token.strip().toLowerCase(Locale.ROOT).replaceAll("[\\s_]+", "_");
  }
  ...
  private static Optional<String> namespaced(Pattern namespace, String token) {
    Matcher matcher = namespace.matcher(Ascii.toLowerCase(token.trim()));
    if (!matcher.matches()) {
