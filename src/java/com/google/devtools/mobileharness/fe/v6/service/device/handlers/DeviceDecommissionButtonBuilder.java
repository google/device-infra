...
/** Utility class to build {@link ActionButtonState} for decommission button. */
@Singleton
class DeviceDecommissionButtonBuilder {

  private final FeatureManagerFactory featureManagerFactory;
  private final FeatureReadiness featureReadiness;

  @Inject
  DeviceDecommissionButtonBuilder(
  ...
  public ActionButtonState build(DeviceInfo deviceInfo, UniverseScope universe) {
    if (!featureManagerFactory.create(universe).isDeviceDecommissionFeatureEnabled()) {
    ...
    boolean isReady = featureReadiness.isDeviceDecommissionReady();
    return ActionButtonState.newBuilder()
        .setVisible(true)
        .setIsReady(isReady)
        .setEnabled(true)
        .setTooltip("Decommission the missing device")
        .build();
  }
}
