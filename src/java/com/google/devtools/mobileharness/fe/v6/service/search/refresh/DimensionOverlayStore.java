 ...
 * in-flight deduplication and returns strong-reference maps for query execution.
 */
@Singleton
public final class DimensionOverlayStore {
  ...
  private final ConcurrentMap<Fleet, ConcurrentMap<String, DimensionOverlay>> memoryCaches =
  ...
  public ListenableFuture<ImmutableMap<String, DimensionOverlay>> loadOverlaysAsync(
    ...
    ConcurrentMap<String, DimensionOverlay> cache =
        memoryCaches.computeIfAbsent(fleet, f -> new ConcurrentHashMap<>());
    ...
    for (String keyId : keyIds) {
      DimensionOverlay cached = cache.get(keyId);
      if (cached != null) {
