...
public interface KeyVocabulary {
  ...
  /**
   * The long-tail key this entity mints for an unresolved bare token, or empty if the token is
   * blank or uses a namespace spelling. Device search mints a dimension, host search mints a host
   * property. This is the last resort of key resolution and lets a user filter on a key the catalog
   * has not discovered yet. A namespaced token that did not resolve names a family this entity
