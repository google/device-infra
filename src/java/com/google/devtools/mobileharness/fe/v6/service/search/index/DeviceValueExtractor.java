 ...
 *
 * <p>This mirrors exactly what the index builder records, so posting lists built from the forward
 * store match the index's sorted values and counts. Used by both {@link LazyPostings} (to build
 * posting lists on demand) and the filter engine (for exact-set matching).
 */
