...
public final class FeGrpcInvoker {
  ...
  private static <RequestT, ResponseT> void handleFailure(
    ...
    if (feException != null) {
      // Server-fault codes (INTERNAL, UNKNOWN, etc.) indicate a bug on our side — log at WARNING
      // with the full cause chain so it's visible for debugging. Client-fault codes (NOT_FOUND,
      // INVALID_ARGUMENT, etc.) are expected and not logged here; the handler that threw already
      // has the context. The internal (AF) server has central framework logging, so this logging
      // is specific to the OSS server which has no such infrastructure.
      if (isServerError(feException.getCode())) {
        logger.atWarning().withCause(feException).log(
            "gRPC service %s server error, service=[%s], method=[%s]",
  ...
  /**
   * Returns {@code true} if the given code represents a server-side fault (HTTP 5xx equivalent)
   * that warrants a WARNING log. Client-fault codes (4xx) are expected and not logged.
   */
  private static boolean isServerError(Status.Code code) {
