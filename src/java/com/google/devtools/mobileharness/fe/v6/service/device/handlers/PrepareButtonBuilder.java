...
/** Utility class to build {@link ActionButtonState} for prepare device button. */
@Singleton
class PrepareButtonBuilder {

  private final FeatureManagerFactory featureManagerFactory;
  private final FeatureReadiness featureReadiness;

  @Inject
  PrepareButtonBuilder(
  ...
  public ActionButtonState build(DeviceInfo deviceInfo, UniverseScope universe) {
    if (!featureManagerFactory.create(universe).isPrepareDeviceFeatureEnabled()) {
    ...
    boolean isFusionDevice =
        deviceInfo.getDeviceFeature().getCompositeDimension().getSupportedDimensionList().stream()
            .anyMatch(
                dimension ->
                    dimension.getName().equals("dm_type")
                        && Ascii.equalsIgnoreCase(dimension.getValue(), "fusion"));

    boolean isReady = featureReadiness.isPrepareDeviceReady();
    if (!isDeviceMissing && isFusionDevice) {
      return ActionButtonState.newBuilder()
          .setVisible(true)
          .setIsReady(isReady)
          .setEnabled(true)
          .setTooltip("Prepare the device (reset to a known good state)")
          .build();
    } else {
      return ActionButtonState.newBuilder().setVisible(false).build();
    }
