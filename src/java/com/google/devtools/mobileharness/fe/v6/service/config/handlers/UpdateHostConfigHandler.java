...
public final class UpdateHostConfigHandler {
  ...
  public ListenableFuture<UpdateHostConfigResponse> updateHostConfig(
    ...
    return FluentFuture.from(performSelfLockoutCheck(request, endUser))
        .transformAsync(
            optResponse ->
  ...
  private void validateRequest(UpdateHostConfigRequest request) {
    if (!configServiceCapability.isUniverseSupported(request.getUniverse())) {
      throw new UnsupportedOperationException(
          String.format(
              "Configuration operations are not currently supported for universe '%s' in the"
  ...
  private ListenableFuture<Boolean> isSelfLockout(
      UpdateHostConfigRequest request, Optional<String> endUser) {
    if (request.getOptions().getOverrideSelfLockout() || endUser.isEmpty()) {
      ...
      case DEVICE_CONFIG ->
          builder.setDefaultDeviceConfig(
              updateBasicConfig(
                  ...
                  DeviceConfigConverter.toBasicDeviceConfig(incoming.getDeviceConfig()),
  ...
  private void updateDeviceDiscovery(LabConfig.Builder builder, DeviceDiscoverySettings discovery) {
    ...
    if (!discovery.getManekiSpecsList().isEmpty()) {
      builder.setDetectorSpecs(
          DeviceConfigConverter.toInternalDetectorSpecs(discovery.getManekiSpecsList()));
    }
  }
