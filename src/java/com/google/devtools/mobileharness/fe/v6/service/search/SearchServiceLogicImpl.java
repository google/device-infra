 ...
 * synchronous and run entirely over the in-memory snapshot, so each result is returned in an
...
public final class SearchServiceLogicImpl implements SearchServiceLogic {
  ...
  public ListenableFuture<FleetSearchResults> searchFleet(FleetSearchRequest request) {
    ...
    return switch (request.getViewCase()) {
      case FLAT -> {
        ...
        FleetFlatResults results =
            flatSearcher.searchFlat(
                ...
                flat.getSort(),
                flat.getPage());
        yield immediateFuture(FleetSearchResults.newBuilder().setFlat(results).build());
      }
      case GROUP_HEADER -> {
        ...
        FleetGroupedResults results =
            groupSearcher.searchGrouped(
                ...
                header.getSort(),
                header.getPage());
        yield immediateFuture(FleetSearchResults.newBuilder().setGrouped(results).build());
      }
      case GROUP_EXPAND -> {
      ...
      // guessing a view.
      case VIEW_NOT_SET -> immediateFuture(FleetSearchResults.getDefaultInstance());
    };
  }
  ...
  public ListenableFuture<FleetSuggestionResponse> getFleetSuggestions(
    ...
    FleetSnapshot snapshot = store.get(normalize(request.getFleet()));
    return immediateFuture(suggester.suggest(snapshot, request));
  }

  @Override
  public ListenableFuture<FleetChipResolverResponse> resolveFleetChips(
      FleetChipResolverRequest request) {
    // Chip resolution is stateless: the request carries no fleet, and the resolver reads only key
    // display names and value casing, which are the same across fleets. Read the self snapshot.
    FleetSnapshot snapshot = store.get(Fleet.FLEET_SELF);
    return immediateFuture(chipResolver.resolve(snapshot, request));
  }
  ...
  public ListenableFuture<FleetValueListResponse> getFleetValueList(FleetValueListRequest request) {
    ...
    return immediateFuture(
        valueLister.listValues(snapshot, request.getKey(), request.getFiltersList()));
  }
  ...
  public ListenableFuture<FleetPromotedKeysResponse> getFleetPromotedKeys(
    ...
    FleetSnapshot snapshot = store.get(normalize(request.getFleet()));
    return immediateFuture(promotedKeysProvider.getPromotedKeys(snapshot, request));
  }
  ...
  public ListenableFuture<FleetColumnCatalogResponse> getFleetColumnCatalog(
    ...
    FleetSnapshot snapshot = store.get(normalize(request.getFleet()));
    return immediateFuture(columnCataloger.getColumnCatalog(snapshot, request));
  }
