...
public final class FleetDataRefresherTest {
  ...
  private FleetDataRefresher refresher(FleetDataSource... sources) {
    ...
    return new FleetDataRefresher(
        map.buildOrThrow(), indexBuilder, store, InstantSource.fixed(BUILD_TIME));
  }
  ...
  /** In-memory {@link FleetDataSource} that returns a configured result or throws. */
  private static final class FakeFleetDataSource implements FleetDataSource {
    ...
    @Override
    public FleetRawData pull() {
      if (failure != null) {
        throw failure;
      }
      return FleetRawData.ofLabData(result);
    }
  }
}
