...
public class CommandResult {
   ...
   * com.google.devtools.deviceinfra.shared.util.command.LineCallback.Response response} of {@link
   * LineCallback} in {@link Command#onStdout(LineCallback)} or {@link
   * Command#onStderr(LineCallback)} which has stopped the command
   *
  ...
  private static String truncateOutput(String output) {
    if (output.length() < CommandResult.MAX_OUTPUT_LENGTH_IN_MSG) {
      return output;
    }

    int startLength = CommandResult.MAX_OUTPUT_LENGTH_IN_MSG / 2;
    String start = output.substring(0, startLength);

    // the extra 5 is taken off for the eclipses added below.
    int endLength = CommandResult.MAX_OUTPUT_LENGTH_IN_MSG / 2 - 5;
