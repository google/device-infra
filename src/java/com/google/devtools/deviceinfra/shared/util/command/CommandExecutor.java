...
import com.google.devtools.deviceinfra.shared.util.command.history.CommandRecord;
...
public class CommandExecutor {
   ...
   * run(Command.of("echo", "Hello"))</code> will return "Hello\n" rather than "Hello". If you do
   * not want trailing line terminator, please use <code>
   * exec(Command).stdoutWithoutTrailingLineTerminator()
   * </code> instead.
   *
  ...
  public CommandProcess start(Command command) throws CommandStartException {
    ...
    Duration remainingTime = getRemainingTime(command.getTimeout().orElse(getDefaultTimeout()));
    Optional<Timeout> startTimeout = command.getStartTimeout();
    Optional<Duration> startRemainingTime = startTimeout.map(CommandExecutor::getRemainingTime);
    ...
    Runnable timeoutTask = new TimeoutTask(commandProcess);
    Function<Duration, Future<?>> timeoutTaskScheduler =
        timeout -> timer.schedule(timeoutTask, timeout.toMillis(), TimeUnit.MILLISECONDS);
    ...
    threadPool.execute(() -> readOutput(stdoutReader, stdoutCollector, commandProcess.command()));
    threadPool.execute(
        () ->
            readOutput(
                stderrReader,
  ...
  // 'com.google.devtools.deviceinfra.shared.util.command.Command' is not annotated with
  ...
  private static Duration getRemainingTime(Timeout timeout) {
  ...
  private static class LazyLoader {
    ...
    private static final ListeningExecutorService DEFAULT_THREAD_POOL = getDefaultThreadPool();

    private static final ListeningScheduledExecutorService DEFAULT_TIMER = getDefaultTimer();
    ...
    private static ListeningExecutorService getDefaultThreadPool() {
       return DEFAULT_NON_PROPAGATING_THREAD_POOL;
    }

    private static ListeningScheduledExecutorService getDefaultTimer() {
      return MoreExecutors.listeningDecorator(
          Executors.newScheduledThreadPool(
              3, getThreadFactory("default-mh-command-executor-timer")));
    }
  }
}
