  ...
  public static ActionSpec getActionSpec(ActionOptions options) throws DeviceActionException {
    return switch (options.command()) {
      case INSTALL_MAINLINE -> {
        ...
        yield getActionSpecForInstallMainline(installMainlineSpec, getUuid(options.firstDevice()));
      }
      case RESET -> {
        Conditions.checkArgument(
            options.firstDevice() != null,
        ...
        yield getActionSpecForReset(resetSpec, getUuid(options.firstDevice()));
      }
      default ->
