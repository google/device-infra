...
public class InstallApkStep implements InstallApkStepConstants {
  ...
  private void installApkSet(
    ...
    ApkSet.Builder apkSetBuilder =
        ApkSet.builder()
            ...
            .addAllDeviceGroups(StrUtil.toList(spec.getApkSetDeviceGroups()));
  ...
  private static InstallApkStepSpec createInstallApkStepSpec(JobInfo jobInfo)
    ...
    if (jobInfo.params().has(PARAM_APK_SET_DEVICE_GROUPS)) {
      spec.setApkSetDeviceGroups(jobInfo.params().get(PARAM_APK_SET_DEVICE_GROUPS));
    }
    return spec.build();
  }
  ...
  private static class PackageToInstall {
    ...
    boolean isApkSet() throws MobileHarnessException {
      if (!apkPaths.get(0).endsWith(".apks")) {
        return false;
      }
      if (dexMetadataPaths.size() > 0) {
        throw new MobileHarnessException(
            AndroidErrorId.ANDROID_INSTALL_APK_STEP_INVALID_APK_SET_ARCHIVE,
            "Apk Set archive cannot have dex metadata files.");
      }
      if (apkPaths.size() > 1) {
        throw new MobileHarnessException(
            AndroidErrorId.ANDROID_INSTALL_APK_STEP_INVALID_APK_SET_ARCHIVE,
            "Apk Set archive cannot have multiple .apks files.");
      }
      return true;
    }
  }
