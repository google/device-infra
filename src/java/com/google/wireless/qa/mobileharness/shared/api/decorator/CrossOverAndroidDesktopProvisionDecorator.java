...
public class CrossOverAndroidDesktopProvisionDecorator extends CrosBaseDecorator {
  ...
  public static final String FOIL_PROVISION_PACKAGE =
      "chromiumos/infra/cft/foil-provision/${platform}";
  public static final String FOIL_PROVISION_CIPD_PATH =
      "/usr/local/google/mobileharness/ate/cipd_packages/foil-provision";
  public static final String FOIL_PROVISION_CIPD_TAG = "foil_provision_cipd_tag";
  public static final String BUILD_ID = "build_id";
  public static final String BUILD_TARGET = "build_target";
  public static final String USE_SIGNED_IMAGE = "use_signed_image";
  public static final String USE_TEST_RAMDISK = "use_test_ramdisk";
  public static final String SKIP_STABLE_VERSION = "skip_stable_version";
  public static final Duration DEFAULT_FOIL_PROVISION_TIMEOUT = Duration.ofMinutes(60);
  ...
  protected SetupResult setUp(SetupContext context)
    ...
    CommandResult result = commandExecutor.exec(provisionCommand);
    if (result.exitCode() != 0) {
      throw new MobileHarnessException(
          BasicErrorId.NON_MH_EXCEPTION,
          String.format(
              "foil-provision failed with exit code %d: %s\nStderr: %s",
  ...
  private BuildInfo resolveBuildParameters(TestInfo testInfo, String dutName)
    ...
    if ("true".equalsIgnoreCase(skipStableVersion)) {
      testInfo
          .log()
          .atInfo()
          .alsoTo(logger)
          .log("Skipping stable version resolution (skip_stable_version is true).");
      return new BuildInfo(buildId, buildTarget);
    }
    ...
    CommandResult result = commandExecutor.exec(cmd);
    if (result.exitCode() != 0) {
      throw new MobileHarnessException(
          BasicErrorId.COMMAND_EXEC_FAIL,
          String.format(
              "Failed to query stable version for %s via dt-converter (exit code %d).\n"
                  + "Stdout: %s\n"
                  + "Stderr: %s",
              dutName, result.exitCode(), result.stdout(), result.stderr()));
    }
  ...
  /** Parses JSON output from dt-converter stable-version and updates missing test parameters. */
  @VisibleForTesting
        ...
        if (root.has("android") && root.get("android").isJsonObject()) {
          ...
          if (androidObj.has("build_id") && !androidObj.get("build_id").isJsonNull()) {
            ...
            if (!id.isEmpty() && Strings.isNullOrEmpty(resolvedBuildId)) {
              resolvedBuildId = id;
              testInfo.jobInfo().params().add(BUILD_ID, id);
              if (testInfo.properties() != null) {
          ...
          if (androidObj.has("build_target") && !androidObj.get("build_target").isJsonNull()) {
            ...
            if (!target.isEmpty() && Strings.isNullOrEmpty(resolvedBuildTarget)) {
              ...
              testInfo.jobInfo().params().add(BUILD_TARGET, target);
              if (testInfo.properties() != null) {
  ...
  Command generateProvisionCommand(
    ...
    if ("true".equalsIgnoreCase(getParam(testInfo, USE_SIGNED_IMAGE, "false"))) {
      args.add("-use-signed-image");
    }
    if ("true".equalsIgnoreCase(getParam(testInfo, USE_TEST_RAMDISK, "false"))) {
  ...
  /** Returns the resolved path to the foil-provision binary. */
  @VisibleForTesting
  String getResolvedFoilProvisionPath() {
    return resolvedFoilProvisionPath;
  }

  /** Returns the resolved path to the dt-converter binary. */
  @VisibleForTesting
  String getResolvedDtConverterPath() {
    return resolvedDtConverterPath;
  }

  /** Returns the temporary directory used for downloading foil-provision, if any. */
  @VisibleForTesting
  @Nullable
  Path getFoilProvisionDownloadedDir() {
    return foilProvisionDownloadedDir;
  }

  /** Returns the temporary directory used for downloading dt-converter, if any. */
  @VisibleForTesting
  @Nullable
  Path getDtConverterDownloadedDir() {
    return dtConverterDownloadedDir;
  }
