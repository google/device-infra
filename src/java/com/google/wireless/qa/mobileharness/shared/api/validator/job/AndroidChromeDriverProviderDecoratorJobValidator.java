...
public class AndroidChromeDriverProviderDecoratorJobValidator
  ...
  @VisibleForTesting
  static final String ERROR_CUSTOM_CHROMEDRIVER_NOT_ALLOWED =
      "Uploading a custom chromedriver binary or specifying chromedriver_path is not allowed;"
          + " ChromeDriver is resolved automatically by the platform.";

  @CanIgnoreReturnValue
  @Override
  public List<String> validate(JobInfo job) throws InterruptedException {
    List<String> errors = new ArrayList<>();
    validateNoCustomChromeDriver(job, errors);
    validateTargetType(job, errors);
    return errors;
  }

  private static void validateNoCustomChromeDriver(JobInfo job, List<String> errors) {
    if (job.files().isTagNotEmpty(FILE_CHROMEDRIVER)
        || job.files().isTagNotEmpty(FILE_CHROMEDRIVER_BINARIES)
        || job.params().has("chromedriver_path")) {
      errors.add(ERROR_CUSTOM_CHROMEDRIVER_NOT_ALLOWED);
    }
  }
