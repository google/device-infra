...
public class AndroidTestLoopTest extends BaseDriver
  ...
  public void run(TestInfo testInfo) throws MobileHarnessException, InterruptedException {
    ...
    for (String scenarioId : scenarios) {
      ...
      if (remainingTimeout.isZero() || remainingTimeout.isNegative()) {
        testInfo
            .log()
            .atInfo()
            .alsoTo(logger)
  ...
  private void installCompanionApp(TestInfo testInfo)
    ...
    String apkPath;
    try {
      Optional<String> externalResFile =
          resUtil.getExternalResourceFile(COMPANION_APK_RESOURCE_PATH);
  ...
  private static Duration getScenariosTimeout(TestInfo testInfo, AndroidTestLoopTestSpec spec) {
    ...
    return Comparators.max(defaultTimeout, Duration.ZERO);
  }
